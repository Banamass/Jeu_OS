...
void MenuCharacters::update(){
    ...
    for (int s=0; s< nbCharacters; s++) {
        vga.plot_sprite(ListCharacters::GetCharacter(s, selection == s ? ATT1 : IDLE), ListCharacters::GetWidth(s), ListCharacters::GetHeight(s), 80+100*s, 200-ListCharacters::GetHeight(s), false);
    }
    vga.swapBuffer();
